...
for ( N in sample.sizes ) { # try different sample sizes
  ...
  m = numeric(0)
  for (i in 1:num_samples_at_size){
    x = rnorm(N, mean=0, sd=1)
    m[i] = mean(x)
  }
  
  samples_sd = sd(m)
  mean.sds = c(mean.sds, samples_sd)
...
N = c(1,3,9,81) # the number of i.i.d. variables X we are going to sum
par(mfrow=c(2,2),ps=12) #, pin=c(2,2), fin=c(0.5,0.5))
...
for (n_value in N) {
  ...
  for (n.exp in 1:repeats) { # repeat the experiment!
    ...
    # exponential distribution results in good histogram progression    
    s.values[n.exp] = sum(rexp(n=n_value, rate=1))
  }
  ...
  hist(s.values, breaks=10, xlab='distrubtion sums', col='lightgreen', main=paste('sums of ', n_value, ' i.i.d from exponential dist.' ))
}
...
### Answer
**The sample mean is itself a random variable and it becomes normally distributed when $N$ becomes large, just another example of the CLT.
The average of the distribution of sample means approaches a constant, that constant being the mean of the population of said distribution. In below `print()`s 
the mean of $N$ = 1 million for a chi squared distribution with degrees of freedom = 2, taken as a proxy for the "full"" population, is ~2.0. 
Likewise for similar $N$ for a exponential distribution, where the comparable mean is ~1.0 for full population. In both cases the average of the sample means converges on the 
matching population mean, as $N$ increases.
The series of plots for chi squared sample means shows the average approaching 2.0, where the normal distribution appearing toward the end allows you to calculate the average "by eye".
Similar progression for the exponential distribution.
Likewise the standard deviation of the sample mean distrubition acts as an estimator of the standard deviation of the underlying distribution and will more closely resemble, 
i.e. get closer in value, to that of said distribution as $N$ becomes large.
The shape of the distribution of sample means at limit of large $N$ should be normal/gaussian shaped, i.e. like a bell shaped mound with evenly tapering sides.**
...
print(paste('exponential distribution, rate=1, mean of N =',large_number, ': ', mean(rexp(n=large_number, rate=1))))
print(paste('chi square distribution, df=2, mean of N =',large_number, ': ', mean(rchisq(n=large_number, df=2))))

par(mfrow=c(2,2),ps=12)

N = c(1,3,9,81,900) # the number of i.i.d. variables X we are going to sum 
repeats = 1000
s.values=numeric() # we will use this vector to store the value of the sum in each experiment
s.values2=numeric()
s.values2a=numeric()
...
for (n_value in N) {
  for (n.exp in 1:repeats) { # repeat the experiment!
    ...
    s.values[n.exp] = mean(rexp(n=n_value, rate=1))
    # chi squarked distribution, degrees of freedom = 2
    s.values2[n.exp] = mean(rchisq(n=n_value, df=2))

  }
  
    hist(s.values2, breaks=20, xlab='distrubtion means', col='lightblue', main=paste('means of ', n_value, ' i.i.d from chi squared dist.' ))
    hist(s.values, breaks=10, xlab='distrubtion means', col='lightgreen', main=paste('means of ', n_value, ' i.i.d from exponential dist.' ))
}
