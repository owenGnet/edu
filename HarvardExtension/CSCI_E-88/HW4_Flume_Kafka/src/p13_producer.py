...
def produce_topic_events(thread_number, data, is_debug=False):
    ...
    for line in data:
        ...
        # if want to keep key/partition for problem13, would need 10 userids, which is a possibility
        producer.send(topic_name, value=str.encode(line))
        if is_debug:
